...
def load_model():
     # model = tf.keras.models.load_model(r"C:\Users\DELL\jupyterr\TrainedModel\model_6.keras")
     model = tf.keras.models.load_model(r"C:\Users\HP\OneDrive\Desktop\CS_HINDI\project_3rd\trainedmodel\model_1.keras")
...
def load_lottieurl(url: str):
    ...
    return r.json()  # Return the JSON data if the request succeeds
...
if sel == 'Prediction':
    ...
    if file is None:
        st.write("### Please Upload an Image that contains Hindi Character")
    else:
        ...
        if st.button("Predict"):
            st.header(f"Top {n} Prediction for the given image")

                # Horizontal bar chart for top n predictions with probability
            fig = go.Figure()
            ...
            st.success(
                f"The image is classified as \t  \'{class_name[0]}\' \t with {confidence[0] * 100:.1f} % probability")
...
if sel == 'Get test images':
    ...
    lottie_download = load_lottieurl(
        "https://assets9.lottiefiles.com/packages/lf20_szdrhwiq.json")  # Link for animated image
...
if sel == 'Code':
    with open('DL_MODEL.py', 'r', encoding="utf8") as f:
        code = f.read()
    st.code(code, 'python')
